"""Router para o endpoint de consulta de detalhes de URL encurtada."""
...
async def get_url_details_use_case_dep(
    ...
    o unittest.mock.patch em `app.infrastructure.di.container.get_url_details_use_case`
...
async def get_url_details(
    ...
    """Consulta detalhes de uma URL encurtada pelo short_code.
    ...
    try:
        ...
        logger.info(
            "Detalhes de URL consultados via API",
            extra={"short_code": short_code, "click_count": result.click_count},
        )
        return result
    except UrlNotFoundError:
        logger.info("URL não encontrada via API", extra={"short_code": short_code})
